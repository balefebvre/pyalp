...
from ctypes import byref, c_long, c_ulong, c_void_p, cast, cdll

from .base import Device as BaseDevice
from .. import utils
from ..base.type import *
from ..protocol import Protocol  # TODO get rid of this line.
from ..stimulus.base import Stimulus  # TODO get rid of this line.
...
class Device(BaseDevice):
    ...
    def reconnect(self):
        ...
        ret_val = self._api.AlpDevControl(device_id, control_type, control_value)
    ...
    def put(self, sequence):  # sequence_id, user_array, pic_offset=ALP_DEFAULT, pic_load=ALP_DEFAULT):
        ...
        user_array_ptr_ = cast(user_array_, c_void_p)
    ...
    def start(self, sequence, infinite_loop=False):
        ...
        if infinite_loop:
            ...
            ret_val_ = self._api.AlpProjStartCont(device_id_, sequence_id_)
            if ret_val_ == ALP_OK:
                return
            else:
    ...
    def wait(self, infinite_loop=False, active=True):
        ...
        if infinite_loop:
            ...
            device_id_ = c_ulong(self.id)
            ret_val_ = self._api.AlpProjHalt(device_id_)
            if ret_val_ == ALP_OK:
                return
            else:
    ...
    def invert_projection(self):
        ...
        ret_val_ = self._api.AlpProjControl()
        if ret_val_ == ALP_OK:
            return
        else:
    ...
    def _control_projection(self, control_type, control_value):
        ...
        control_type_ = c_long(control_type)
        control_value_ = c_long(control_value)
        ret_val_ = self._api.AlpProjControl(device_id_, control_type_, control_value_)
        if ret_val_ == ALP_OK:
            return
        else:
            raise Exception("AlpProjControl: {}".format(ret_val_))
    ...
    def inquire_projection(self, inquire_type):
        ...
        if inquire_type is 'progress':
            ...
            ret_val_ = self._api.AlpProjInquireEx(device_id_, inquire_type_, user_struct_ptr_)
            if ret_val_ == ALP_OK:
